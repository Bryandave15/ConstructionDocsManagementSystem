...
<div style="display: flex; justify-content: space-around; border-bottom: 2px solid black; padding-bottom: 2px;">
    <h2 style="margin: 0; margin-right: 200px">AS -BUILT</h2>
    <!-- Headline "FORMS" --> 
    <a class="btn btn-primary"  href="/asbuilt-add-form"><b>Add Drawing</b></a>
        <!-- Search input field -->
...
<thead>
        <tr>
            <th style="border-bottom: 2px solid black;">As-built Title</th>
...
@foreach ($asbuilt_data as $d)
<tr >

    <td>{{ $d->asbuilt_title }}</td>
    <td>{{ $d->asbuilt_code }}</td> 
    <td>{{ $d->asbuilt_location }}</td> 
    <td>{{ $d->trade }}</td> 
    <td>
    ...
    <td>
        ...
        <button id="butEdit" onclick="processEdit({{ $d->asbuilt_id }})" class="btn btn-success">Update</button>
        <button id="butDelete" onclick="processDelete({{ $d->asbuilt_id }})" class="btn btn-danger">Delete</button>
...
{
    window.location.href = '/asbuilt-update-form/'+pId 
}
